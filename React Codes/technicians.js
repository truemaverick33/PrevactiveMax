...
useEffect(()=>{
		$.ajax({type:"POST",url:connstr+"/Backend/Select.php",data:{datafor:"technicians"},success(data){
			var jsonData=JSON.parse(data);
			var act ="";
			var blc ="";
			for(let i=0; i<jsonData.length ; i++){
				if(jsonData[i].status == 1){
				act+="<tr class='dept' id='"+jsonData[i].userNo+"' style='cursor:grab'><td>"+jsonData[i].userNo+"</td><td>"+jsonData[i].employeeName+"</td><td>"+jsonData[i].employeeId+"</td><td>"+jsonData[i].dept+"</td><td>"+jsonData[i].type+"</td><td border='0'><div class='card' align='center' id='m"+jsonData[i].userNo+"' style='display:none;width:7rem;padding:3px'></div></div></div></td></tr>";
				}
			}
...
<h4 className="display-4" align="center">Technicians</h4>
<Accordion defaultActiveKey="0">
<Accordion.Item eventKey="0">
<Accordion.Header>Active Users</Accordion.Header>
<Accordion.Body>
<Table responsive>
<thead className="table-dark"><tr><th>UserNo.</th><th>Technician Name</th><th>EmployeeId</th><th>Department</th><th>Type</th><th></th></tr></thead>
<tbody id="atc"></tbody>
</Table>
</Accordion.Body>
</Accordion.Item>
