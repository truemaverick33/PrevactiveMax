...
useEffect(()=>{
		$.ajax({type:"POST",url:connstr+"/Backend/Select.php",data:{datafor:"depta"},success(data){
			...
			for(let i=0; i<jsonData.length ; i++){
				...
				act+="<tr class='depta' id='"+jsonData[i].deptId+"' style='cursor:grab'><td>"+jsonData[i].deptId+"</td><td>"+jsonData[i].deptName+"</td><td border='0'><div class='card' align='center' id='m"+jsonData[i].deptId+"' style='display:none;width:5rem;padding:3px'></div></div></div></td></tr>";
			}
		...
		for(let x=0; x<deps.length;x++){
			deps[x].addEventListener("mousedown",function(){
				showMenu(deps[x].id,"departments");
			});
		}
		for(let x=0; x<deps.length;x++){
		...
		$.ajax({type:"POST",url:connstr+"/Backend/Select.php",data:{datafor:"deptb"},success(data){
			var jsonData=JSON.parse(data);
			var blc ="";
			for(let i=0; i<jsonData.length ; i++){
				blc+="<tr class='deptb' id='"+jsonData[i].deptId+"' style='cursor:grab'><td>"+jsonData[i].deptId+"</td><td>"+jsonData[i].deptName+"</td><td border='0'><div class='card' align='center' id='m"+jsonData[i].deptId+"' style='display:none;width:3rem;padding:3px'></div></div></div></td></tr>";
		...
		for(let x=0; x<deps.length;x++){
			deps[x].addEventListener("mousedown",function(){
				showMenu2(deps[x].id,"departments");
			});
		}
		for(let x=0; x<deps.length;x++){
			deps[x].addEventListener("mouseup",function(){
...
<div style={{position: 'absolute',top: 15,right: 10,display: 'block'}}><Button variant="light" onClick={()=>{window.location.href="/add/departments"}}><i style={{fontSize:"24px"}} className="bi bi-building-fill-add"></i></Button></div>
<h4 className="display-4" align="center">Departments</h4>
<Accordion defaultActiveKey="0">
<Accordion.Item eventKey="0">
<Accordion.Header>Active Departments</Accordion.Header>
<Accordion.Body>
<Table responsive>
<thead className="table-dark"><tr><th>Department Id</th><th>Department Name</th><th></th></tr></thead>
<tbody id="atc"></tbody>
</Table>
</Accordion.Body>
</Accordion.Item>
<Accordion.Item eventKey="1">
<Accordion.Header>Blocked Departments</Accordion.Header>
<Accordion.Body>
<Table responsive>
<thead className="table-dark"><tr><th>Department Id</th><th>Department Name</th><th></th></tr></thead>
<tbody id="blc"></tbody>
